...
\textbf{between geo-active users}&\\
\end{tabular}
\caption{\small Summary statistics of the data collected from NYC. Geo-active users are those who geo-tag their tweets (i.e., include the location from where they tweet) relatively frequently (more than 100 times per month).} 
\label{table::dataset}
\end{table}
...
In order to test the effectiveness of various methods of capturing training data, we used two different methods to filter for tweets that are relatively likely to center on suicide risk factors (Table~\ref{tab:filter}). The first method, we used the Linguistic Inquiry and Word Count (LIWC) to... capture 1,370 tweets. LIWC has been widely used to estimate emotion in online social networks, and specifically capture mood. 
